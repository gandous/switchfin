...
Home::Home(std::shared_ptr<JellyfinClient> client): _jclient(client)
{
    ...
    scroll->set_size(gana::Vector2f(700, 300));
    add_child(scroll);
...
void Home::on_resume_receive(gana::Request::RCode code, gana::Request &req)
{
    ...
    for (auto &item: _rresume->get_items()) {
        BigMovieVignette *vign = _ctn_main.make_managed<BigMovieVignette>(_jclient->get_http(), _jclient->get_img_url(item.get_id(), JellyfinClient::BACKDROP));
        gana::Logger::info("Name: %s", item.get_name().c_str());
        _ctn_main.add_child(vign);
    }
